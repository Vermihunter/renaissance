// Parameter generator for tw-8.cypher
// Generated for the Renaissance Neo4j LDBC/FinBench benchmark.
//
// Contract:
//   - Return one row per parameter set.
//   - Column names must match the parameters used by tw-8.cypher.
...
MATCH (acc:Account), (loan:Loan)
WITH acc, loan, currentTime
ORDER BY acc.id ASC, loan.id ASC
LIMIT 15
RETURN
  acc.id AS accountId,
  loan.id AS loanId,

// Parameter generator for tcr-1.cypher
// Generated for the Renaissance Neo4j LDBC/FinBench benchmark.
//
// Contract:
//   - Return one row per parameter set.
//   - Column names must match the parameters used by tcr-1.cypher.
...
MATCH
  p=(account:Account)-[edge1:transfer*1..3]->(other:Account),
  (other)<-[edge2:signIn]-(medium:Medium {isBlocked: true})
WITH account, start_time, end_time, p, edge1, edge2,
...
WHERE
  ...
  AND all(e IN edge1 WHERE start_time < e.timestamp AND e.timestamp < end_time)

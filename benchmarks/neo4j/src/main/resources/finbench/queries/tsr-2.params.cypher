// Parameter generator for tsr-2.cypher
// Generated for the Renaissance Neo4j LDBC/FinBench benchmark.
//
// Contract:
//   - Return one row per parameter set.
//   - Column names must match the parameters used by tsr-2.cypher.
...
MATCH (src:Account)-[e:transfer]-(:Account)
WHERE start_time < e.timestamp AND e.timestamp < end_time
RETURN
  src.id AS id,
  start_time AS start_time,
  end_time AS end_time,
  count(e) AS _matchingRows

// Parameter generator for tw-14.cypher
// Generated for the Renaissance Neo4j LDBC/FinBench benchmark.
//
// Contract:
//   - Return one row per parameter set.
//   - Column names must match the parameters used by tw-14.cypher.
...
MATCH (acc:Account)-[:repay]->(loan:Loan)-[:deposit]->(acc)
RETURN acc.id AS id, count(loan) AS _matchingLoans
ORDER BY _matchingLoans DESC, id ASC
LIMIT 15

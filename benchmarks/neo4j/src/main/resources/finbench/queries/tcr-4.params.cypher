// Parameter generator for tcr-4.cypher
// Generated for the Renaissance Neo4j LDBC/FinBench benchmark.
//
// Contract:
//   - Return one row per parameter set.
//   - Column names must match the parameters used by tcr-4.cypher.
...
MATCH
  (src:Account)-[edge1:transfer]->(dst:Account),
...
WITH src, dst, start_time, end_time, count(*) AS _matchingRows,
     sum(edge2.amount) + sum(edge3.amount) AS _score
RETURN
  src.id AS id1,
  dst.id AS id2,
  ...
  _matchingRows,
  _score
ORDER BY _score DESC, _matchingRows DESC, id1 ASC, id2 ASC
LIMIT 15

// Parameter generator for tcr-2.cypher
// Generated for the Renaissance Neo4j LDBC/FinBench benchmark.
//
// Contract:
//   - Return one row per parameter set.
//   - Column names must match the parameters used by tcr-2.cypher.
...
MATCH
  (person:Person)-[:own]->(accounts:Account),
  p=(accounts)<-[edge2:transfer*1..3]-(other:Account),
  (other)<-[edge3:deposit]-(loan:Loan)
WITH person, start_time, end_time, p, edge2, edge3,

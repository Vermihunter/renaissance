// Parameter generator for tcr-5.cypher
// Generated for the Renaissance Neo4j LDBC/FinBench benchmark.
//
// Contract:
//   - Return one row per parameter set.
//   - Column names must match the parameters used by tcr-5.cypher.
...
MATCH
  (person:Person)-[:own]->(src:Account),
  p=(src)-[edge2:transfer*1..3]->(dst:Account)
WITH person, start_time, end_time, p, edge2,
     [e IN relationships(p) | e.timestamp] AS ts
WHERE
      reduce(curr = head(ts), x IN tail(ts) |
        CASE WHEN curr < x THEN x ELSE 9223372036854775807 END
      ) <> 9223372036854775807
  AND all(e IN edge2 WHERE start_time < e.timestamp AND e.timestamp < end_time)
RETURN
  person.id AS id,
  start_time AS start_time,
  end_time AS end_time,
  count(*) AS _matchingRows,
  max(length(p)) AS _maxPathLength
ORDER BY _matchingRows DESC, _maxPathLength DESC, id ASC
LIMIT 15
